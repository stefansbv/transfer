...
subtest 'Command "run" with full options' => sub {
    MooseX::App::ParsedArgv->new( argv => [qw(run --dryrun --verbose --if t/siruta.xls --recipe t/recipes/recipe-xls.conf)] );
    ...
    is( $app->recipe_file, 't/recipes/recipe-xls.conf', 'Option "--recipe" is set' );
};

subtest 'Command "run" without optional options' => sub {
    MooseX::App::ParsedArgv->new(argv => [qw(run --if t/siruta.xls --recipe t/recipes/recipe-xls.conf)]);
    my $app = App::Transfer->new_with_command();
    isa_ok($app, 'App::Transfer::Command::run');
...
subtest 'Command "run" without options' => sub {
    ...
    is( $app->blocks->[0]->header,
        "Required option 'recipe' missing",
        'Message 1 is set'
    );
